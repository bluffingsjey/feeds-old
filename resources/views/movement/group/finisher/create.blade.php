    ...
    <div class="col-sm-6 col-md-12">
        <div class="panel panel-primary">
            <div class="panel-heading">
                <strong>Create Finisher Group</strong>
                <a href="{{url('/finisher')}}" class="btn btn-warning btn-xs pull-right" role="button">Back</a>
            </div>
            <div class="panel-body holder">
            	<div class="col-md-offset-2 col-md-8">
                    <form class="form-horizontal" action="{{url('/saveFinisher')}}" method="post" id="nursery_form">
                    	<input id="unique_id" name="unique_id" type="hidden" value="{{$unique_id}}"/>
                      <div class="form-group">
                        <label class="col-sm-4 control-label">Finisher Group</label>
                        <div class="col-sm-5">
                      ...
                      <div class="form-group">
                        <label class="col-sm-4 control-label">Finisher Farm</label>
                        <div class="col-sm-5">
                          <select name="nursery" class="form-control" id="nursery">
                          	@forelse($finisher as $k => $v)
                      ...
                      <div class="form-group" style="display: none">
                        ...
                        <div class="col-sm-offset-4 col-sm-5">
                          <button type="button" class="btn btn-block btn-info btn-add-nursery" data-toggle="modal" data-target="#nurseryModal">Add Nursery Group</button>
                        </div>

                      </div>

                      <div class="AddedFarrowGroups" style="display: none">
...
<!-- Modal -->
<div class="modal fade" id="nurseryModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Nursery Groups</h4>
      </div>
      <div class="modal-body">
      	<form class="form-horizontal">
            <div class="form-group">
                ...
                <div class="col-sm-5">
                 <select id="nurseryGroups" class="form-control">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary btn-add-nursery-group">Add</button>
      </div>
    </div>
  </div>
</div>
...
function emptyPending(){
	...
	$(".btn-add-nursery-group").click(function(e) {

        data = {
			'group_id':$("#nurseryGroups").val(),
			'number_of_pigs':$("#numberOfPigs").val(),
			'unique_id': $("#unique_id").val()
		}
		$.ajax({
			url		:	app_url+'/savePendingSelectionFinisher',
			data	:	data,
			type	:	"POST",
			success	: function(r){
				if(r == "exists"){
	...
	function nurseryBins(farm_id){
        ...
        $(".bins_holder").html("");
				...
				nurseryGroup(farm_id,$("#bin-0").val());
				enableCreate()
				//saveSelecMenuSelection();
			}
		});
	...
	*/
	function disableCreate(){
		...
		$(".btn-add-nursery").attr("disabled",true);
		$(".btn-create-group").attr("disabled",true);
	}
	...
	*/
	function enableCreate(){
		...
		$(".btn-add-nursery").attr("disabled",false);
		$(".btn-create-group").attr("disabled",false);
	}
	...
	*/
	$(".container").delegate(".btn-create-group","click",function(e) {
		...
		//} else if(start_weight > end_weight){
			...
			// check exists
			$.ajax({
				url		:	app_url+"/checkExistsFinisher",
				data	: 	{'group_name':$("#group_name").val(),'bins':values},
				type	:	"POST",
				success	: 	function(r){
					if(r == 1){
						alert("Finisher group name existed");
						//counter_catcher.push("fail");
