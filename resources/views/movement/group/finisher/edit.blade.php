<!-- Modal -->
<div class="modal fade" id="editFinisher{{$v['group_id']}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        	...
        	<form class="form-horizontal" method="post" id="finisher_form">
        ...
        <div class="form-group">
			  ...
			  <div class="form-group">
				<label for="inputPassword" class="col-sm-4 control-label">Finisher Farms</label>
				<div class="col-sm-5">
				  <select name="finisher" class="form-control" id="finisher{{$v['group_id']}}">
...
$(document).ready(function(e) {
	...
	finisherFarms({{$v['farm_id']}})

	// append the finisher farms
	function finisherFarms(farm_id){

		$.ajax({
			url		:	app_url+"/finisherfarms",
			data	: 	"",
			type	:	"GET",
			success	: 	function(r){
				...
				$("#finisher"+{{$v['group_id']}}).html("");
				$("#finisher"+{{$v['group_id']}}).append(select_options);
			}
		});
  ...
  $("#finisher{{$v['group_id']}}").change(function(e) {
    	id = $(this).val();
  		//hide bins
  ...
  var loading_list = "<div style='width:500px; margin-left:200px; margin:0 auto; padding-top: 40px; padding-bottom: 40px;'>";
    ...
    var end_weight = parseFloat($(".end_weight_{{$v['group_id']}}").val());
		// farm
		var finisher_farm = $("#finisher{{$v['group_id']}}").val();
		// unique_id
    ...
    var f_bins_id = $('.bins_{{$v['group_id']}}').map(function() {
      ...
      'f_bins_id' : f_bins_id
		...
		updateFinisher(animal_group,start_weight,end_weight);
    });

	function updateFinisher(animal_group,start_weight,end_weight){
		...
		//} else if(start_weight > end_weight){
			...
			// check exists
			$.ajax({
				url		:	app_url+"/updatefinisher",
				data	: 	animal_group,
				type	:	"POST",
				success	: 	function(r){
          if(r == 'success'){

						$('#editFinisher{{$v['group_id']}}').modal('hide');

            setTimeout(function(){
              $(".finisher-list").html("");
              $(".finisher-list").append(loading_list);
            },500);
