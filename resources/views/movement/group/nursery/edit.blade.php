<!-- Modal -->
<div class="modal fade" id="editNursery{{$v['group_id']}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        	...
        	<form class="form-horizontal" method="post" id="nursery_form">
        ...
        <div class="form-group">
			  ...
			  <div class="form-group">
				<label for="inputPassword" class="col-sm-4 control-label">Nursery Farms</label>
				<div class="col-sm-5">
				  <select name="nursery" class="form-control" id="nursery{{$v['group_id']}}">
        ...
        <!--
			  <div class="form-group">
				...
				<div class="col-sm-5">
				  <input name="number_of_pigs_one" type="number" class="form-control" id="pigs_{{$v['group_id']}}" value="{{$v['total_pigs']}}">
				</div>
			  </div>
      -->
			</form>
...
$(document).ready(function(e) {
	...
	nurseryFarms({{$v['farm_id']}})

	// append the nursery farms
	function nurseryFarms(farm_id){

		$.ajax({
			url		:	app_url+"/nurseryfarms",
			data	: 	"",
			type	:	"GET",
			success	: 	function(r){
				...
				$("#nursery"+{{$v['group_id']}}).html("");
				$("#nursery"+{{$v['group_id']}}).append(select_options);
			}
		});
  ...
  $("#nursery{{$v['group_id']}}").change(function(e) {
    	id = $(this).val();
  		//hide bins
  ...
  var loading = "<div style='width:250px; height:50px; margin-left:200px;'>";
  ...
  var loading_list = "<div style='width:500px; margin-left:200px; margin:0 auto; padding-top: 40px; padding-bottom: 40px;'>";
	...
	$(".btn-save-changes-{{$v['group_id']}}").click(function(e) {
      ...
      var end_weight = parseFloat($(".end_weight_{{$v['group_id']}}").val());
  		// farm
  		var nursery_farm = $("#nursery{{$v['group_id']}}").val();
  		// unique_id
      ...
      var f_bins_id = $('.bins_{{$v['group_id']}}').map(function() {
        ...
        'f_bins_id' : f_bins_id
  		...
  		updateNursery(animal_group,start_weight,end_weight);
  });

	function updateNursery(animal_group,start_weight,end_weight){

		if($("#pigs").val() == ""){
			alert("Please enter the pigs.");
		} else if($("#group_name").val() == ""){
			alert("Please enter the group name.");
		//} else if($(".start_weight_{{$v['group_id']}}").val() == "" || start_weight == 0){
		//	alert("Please enter the start weight.");
		//} else if($(".end_weight_{{$v['group_id']}}").val() == "" || end_weight == 0){
		...
		//} else if(start_weight > end_weight){
          ...
          if(r == 'success'){
						...
						$('#editNursery{{$v['group_id']}}').modal('hide');

            setTimeout(function(){
              $(".nursery-list").html("");
              $(".nursery-list").append(loading_list);
            },500);
	...
	function loadBinsEdit(key,id,farm_id,bin_id,number_of_pigs){
		...
		$.ajax({
          ...
          select_options += "</div>";
				...
				//saveSelecMenuSelectionEdit{{$v['group_id']}}();
