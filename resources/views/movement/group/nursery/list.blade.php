    ...
    <div class="col-sm-6 col-md-12">
        <div class="panel panel-primary">
            <div class="panel-heading">
                <strong>Nursery Groups</strong>
                <a href="{{url('/createnursery')}}" class="btn btn-success btn-xs pull-right" role="button">Create Group</a>
            </div>
            <div class="panel-body nursery-list">

                <div class="row">

                    @forelse($nursery_data as $k => $v)
                        ...
                        <div class="panel panel-info" style="min-height: 230px;">
                            <div class="panel-heading">
                                ...
                                <h3 class="panel-title text-left"><strong>{{$v['group_name']}}</strong>
                                	<button type="button" class="btn btn-danger btn-xs pull-right btn-delete" nursery-id="{{$v['group_id']}}"  aria-label="Left Align"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>

                                	<button type="button" class="btn btn-warning btn-xs pull-right btn-edit" group-id="{{$v['group_id']}}" farm-id="{{$v['farm_id']}}" bin-id="" aria-label="Left Align" data-toggle="modal" data-target="#editNursery{{$v['group_id']}}" style="margin-right: 2px;">
                    ...
                    @include('movement.group.nursery.edit')
										...
										<div class="col-sm-12 col-md-12 loadmore-button-holder">
												<div class="panel panel-info">
														<div class="panel-heading loading-holder">
																<button type="button" class="btn btn-default btn-block btn-loadmore" items="{{count($nursery_data)}}" counter="{{$nursery_counter}}"><strong>Load More...</strong></h3>
														</div>
												</div>
										</div>
...
$(document).ready(function(e) {
	...
	$(".container").delegate(".btn-delete","click",function(e) {
		...
		nursery_id = $(this).attr("nursery-id");

		$.ajax({
			'url'	:	app_url+"/removeGroupNursery",
			data	: 	{'nursery_id':nursery_id},
			type	:	"POST",
			success	: 	function(r){
				window.location.reload();
				$(".group-"+nursery_id).hide();
			}
		})
